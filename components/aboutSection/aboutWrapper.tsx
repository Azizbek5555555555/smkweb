...
export function AboutWrapper({}) {
  const [text, setText] = useState({
    main: "Избранные работы",
    para: `Мы создаем высококлассные веб-сайты, которые будут полезны и востребованы предприятиями и их владельцами.`,
  });
  useEffect(() => {
    if (!isDesktop()) {
      setText({
        main: "Недавние работы",
        para: `Мы создаем высококлассные веб-сайты, которые будут полезны и востребованы предприятиями и их владельцами.`,
      });
    }
  }, []);
    ...
    <main className="flex h-full w-full max-w-maxWidth grow flex-col justify-center text-[5.8vw] md:text-[clamp(20px,_1vw_+_14px,_32px)]  ">
      <div className="anime relative flex flex-col gap-[1em] md:flex-row-reverse md:gap-[2em] ">
        ...
        <Magentic
          href={links.work}
          scrambleParams={{
            text: "Посмотреть наше портфолио",
          }}
          onMouseEnter={() => {
            if (isDesktop()) {
              gsap.to("body", {
